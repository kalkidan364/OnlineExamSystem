...
// Stats for sidebar summary card
const summaryStats = computed(() => ({
  todays: upcomingExams.value.length > 0 ? 1 : 0,
  ongoing: activeExam.value ? 1 : 0,
  completed: results.value.length > 0 ? results.value.length : 7,
  total: (upcomingExams.value.length || 1) + (activeExam.value ? 1 : 1) + (results.value.length || 7),
}))
...
<template>
  <div class="min-h-screen bg-[#f5f6fa] font-sans text-slate-800 antialiased flex flex-col">
    ...
    <HeroSection :profile="profile" :stats="{ examsCompleted: completedCount, upcomingExams: upcomingExams.length }" />
      ...
      <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="flex-1 space-y-6">
          ...
          <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <select class="text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-2xl px-4 py-2.5 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-300">
          ...
          <!-- ── Section 1: Today's Exam Card (Red Border & Green Action Button Border) ── -->
          <div v-if="activeFilter === 'todays' || activeFilter === 'all'" class="bg-gradient-to-br from-red-50/30 via-white to-white rounded-3xl border-2 border-red-600/80 shadow-md p-6 space-y-5 ring-4 ring-red-50/50">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p class="text-[11px] text-slate-400">June 20, 2025</p>
                </div>
              </div>
            ...
            <!-- Today's Exam Card Content -->
            <div class="bg-white/90 rounded-2xl border border-indigo-100 p-5 shadow-2xs">
              <div class="flex flex-wrap md:flex-nowrap items-center justify-between gap-6 pb-5 border-b border-slate-100">
                ...
                <div class="flex items-center gap-4 min-w-[240px]">
                  ...
                  <div>
                    <h3 class="text-base font-bold text-slate-900 leading-tight">Data Structures</h3>
                    <p class="text-xs text-slate-500 mt-0.5">CS305 - Midterm Exam</p>
                ...
                <div class="space-y-1 text-xs text-slate-600">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium text-slate-700">09:00 AM – 11:00 AM</span>
                  </div>
                  <div class="flex items-center gap-2 text-slate-400">
                    <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span>Duration: 2 Hours</span>
                  </div>
                </div>

                <!-- Status & Countdown -->
                <div class="text-right flex-shrink-0">
                  <span class="inline-block text-[11px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full mb-1">
                    Not Started
                  </span>
                  <p class="text-[11px] text-slate-400">Starts in</p>
                  <p class="text-2xl font-black text-slate-900 leading-none">45m</p>
                </div>
              </div>

              <!-- Full-Width Start Exam Button with Green Border (View Details Removed) -->
              <div class="pt-4">
                <button
                  @click="handleStartExam(1)"
                  class="w-full bg-indigo-50 hover:bg-indigo-100 text-emerald-500 border-2 border-emerald-500 font-bold py-3.5 px-6 rounded-2xl shadow-2xs transition-all duration-200 flex items-center justify-center gap-2"
                >
          ...
          <!-- ── Section 2: Completed Exams (Placed directly after Today's Exam Card!) ── -->
          <div v-if="activeFilter === 'completed' || activeFilter === 'all'" class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
            ...
            <!-- Card Content -->
            <div class="bg-slate-50/50 rounded-2xl border border-slate-100 p-5 flex flex-wrap md:flex-nowrap items-center justify-between gap-6">
              <!-- Course Info -->
              <div class="flex items-center gap-4 min-w-[220px]">
                ...
                <div>
                  <h3 class="text-base font-bold text-slate-900 leading-tight">Discrete Mathematics</h3>
                  <p class="text-xs text-slate-500 mt-0.5">MATH201 - Final Exam</p>
                  <div class="flex items-center gap-1 mt-1 text-[11px] text-slate-400">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>
                    Lecture Hall 1
                  </div>
              ...
              <div class="text-xs text-slate-500">
                ...
                <p class="font-semibold text-slate-700 mt-0.5">May 18, 2025 – 11:30 AM</p>
              </div>

              <!-- Score -->
              <div class="text-center flex-shrink-0">
                <p class="text-[11px] text-slate-400 font-medium">Score</p>
                <p class="text-2xl font-black text-emerald-500 leading-tight">86%</p>
              </div>

              <!-- Action Button styled like second image -->
              <div class="flex flex-col items-end gap-2 flex-shrink-0">
                <span class="text-[11px] font-bold text-blue-600 bg-blue-50 border border-blue-200 px-3 py-1 rounded-full">
                  Completed
                </span>
                <button class="bg-indigo-50 hover:bg-indigo-100 border border-indigo-200/80 text-indigo-600 text-xs font-bold py-2.5 px-6 rounded-2xl transition-all duration-200">
                  View Results
                </button>
              </div>
            </div>
          ...
          <!-- ── Section 3: Ongoing Exams ── -->
          <div v-if="activeFilter === 'ongoing' || activeFilter === 'all'" class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
            <!-- Header -->
            <div class="flex items-center gap-2.5">
            ...
            <div class="bg-slate-50/50 rounded-2xl border border-slate-100 p-5 flex flex-wrap md:flex-nowrap items-center justify-between gap-6">
              ...
              <div class="flex items-center gap-4 min-w-[220px]">
                ...
                <div>
                  <h3 class="text-base font-bold text-slate-900 leading-tight">Algorithms</h3>
                  <p class="text-xs text-slate-500 mt-0.5">CS401 - Quiz 3</p>
                  <div class="flex items-center gap-1 mt-1 text-[11px] text-slate-400">
              ...
              <div class="text-xs text-slate-500 space-y-1">
                <p>Started: <span class="font-semibold text-slate-700">10:05 AM</span></p>
                <p>Duration: <span class="font-semibold text-slate-700">1 Hour</span></p>
              </div>
              ...
              <div class="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <button
                  @click="router.push('/student/exam/take')"
          ...
          <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              <!-- 1. Take Exam -->
              <div class="p-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-indigo-50/60 hover:border-indigo-100 transition-all cursor-pointer group">
                ...
                <p class="text-[10px] text-slate-400 mt-0.5">Start an available exam</p>
              </div>
              ...
              <div class="p-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-indigo-50/60 hover:border-indigo-100 transition-all cursor-pointer group">
                ...
                <p class="text-[10px] text-slate-400 mt-0.5">Check your latest results</p>
              </div>
              ...
              <div class="p-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-indigo-50/60 hover:border-indigo-100 transition-all cursor-pointer group">
                ...
                <p class="text-[10px] text-slate-400 mt-0.5">Read exam instructions</p>
              </div>
              ...
              <div class="p-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-indigo-50/60 hover:border-indigo-100 transition-all cursor-pointer group">
                ...
                <p class="text-[10px] text-slate-400 mt-0.5">View your exam schedule</p>
              </div>
              ...
              <div class="p-3.5 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-indigo-50/60 hover:border-indigo-100 transition-all cursor-pointer group">
                ...
                <p class="text-[10px] text-slate-400 mt-0.5">Open scientific calculator</p>
              </div>
            </div>
          </div>
        ...
        <div class="w-full lg:w-72 flex-shrink-0 space-y-5">
          ...
          <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-5 space-y-3.5">
            ...
            <button class="w-full mt-2 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200/80 text-indigo-600 text-xs font-bold py-2.5 px-4 rounded-2xl transition-all duration-200">

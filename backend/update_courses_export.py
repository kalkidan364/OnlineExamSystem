...
file_path = 'app/Http/Controllers/Api/V1/AdminCourseController.php'
...
end_str = "        return response($pdfContent, 200, [\n            'Content-Type'        => 'application/pdf',\n            'Content-Disposition' => 'attachment; filename=\"' . $fileName . '.pdf\"',\n            'Pragma'              => 'no-cache',\n            'Cache-Control'       => 'must-revalidate, post-check=0, pre-check=0',\n        ]);\n    }"
end_index = content.find(end_str)

if start_index != -1 and end_index != -1:
    new_content = content[:start_index] + new_methods.strip() + content[end_index + len(end_str):]
